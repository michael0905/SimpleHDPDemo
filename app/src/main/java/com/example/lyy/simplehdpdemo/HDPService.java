...
public class HDPService extends Service {
    ...
    public static final int RECEIVED_DATE = 503;
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case MSG_UNREG_CLIENT:
                    mClient = null;
                    break;
                // Register health application with blood pressure data type.
    ...
    private void registerApp(int dataType) {
        if (!mBluetoothHealth.registerSinkAppConfiguration(TAG, dataType, mHealthCallback))
            Log.i(TAG, "registerApp() fail");
        else Log.i(TAG, "registerApp() success");
    }
    ...
    private void connectChannel() {
        ...
        if (!mBluetoothHealth.connectChannelToSource(mDevice, mHealthAppConfig))
            Log.i(TAG, "connectChannel() fail");
        else Log.i(TAG, "connectChannel() success");
    }
...
//            if (Log.isLoggable(TAG, Log.DEBUG))
    ...
    private void sendMessage(int what, int value, String string) {
        if (mClient == null) {
        ...
        try {
            mClient.send(Message.obtain(null, what, value, 0, string));
